...
const PrescriptionManager = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('prescriptions').select('*').order('created_at', {
        ascending: false
      });
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('prescriptions').upload(filePath, file);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (prescription: Prescription) => {
      const {
        error: storageError
      } = await supabase.storage.from('prescriptions').remove([prescription.file_path]);
  ...
  return <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid gap-3 md:gap-4">
        {prescriptions?.map(prescription => <PrescriptionCard key={prescription.id} prescription={prescription} onDelete={p => deleteMutation.mutate(p)} />)}
      </div>

      {prescriptions?.length === 0 && <div className="text-center py-6 md:py-8 text-muted-foreground">
